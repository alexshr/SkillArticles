...
object MarkdownParser {
    ...
    fun clear(string: String?): String? {
        ...
        findElements(string).forEach { resultString.append(clearString(it)) }
    ...
    private fun clearString(element: Element): String {
        ...
        element.elements.forEach{clearString.append(clearString(it))}
    ...
    private fun findElements(string: CharSequence): List<Element> {
        ...
        //logd("1 string=$string; \nmatcher=$matcher")
        loop@
        while (matcher.find(lastStartIndex)) {
            val startIndex = matcher.start()
            val endIndex = matcher.end()
            //logd()
            //logd("2 lastStartIndex=$lastStartIndex; found: startIndex=$startIndex, endIndex=$endIndex")
            if (lastStartIndex < startIndex) {
                //до startIndex идет обычный текст
                val el = Element.Text(string.subSequence(lastStartIndex, startIndex))
                parents.add(el)
                //logd("3 element added: $el")
            }
            ...
            //logd()
            when (val gr = groups.firstOrNull { matcher.group(it) != null } ?: -1) {
                // NOT FOUND -> break
                -1 -> {
                    //logd("when group=$gr break@loop")
                    break@loop
                }
                ...
                1 -> {
                    ...
                    //logd("when group=$gr; lastStartIndex=$lastStartIndex; text=$text; subElements=$subElements; element: $element")
                }
                ...
                2 -> {
                    ...
                    lastStartIndex = endIndex
                    //logd("when group=$gr; lastStartIndex=$lastStartIndex; text=$text; level=$level; element: $element")
                ...
                7 -> {
                    ...
                    lastStartIndex = endIndex
                    //logd("when group=$gr; lastStartIndex=$lastStartIndex; element: $element")
                ...
                8 -> {
                    ...
                    lastStartIndex = endIndex
                    //logd("when group=$gr; lastStartIndex=$lastStartIndex; text=$text;  element: $element")
                ...
                9 -> {
                    ...
                    lastStartIndex = endIndex
                    //logd("when group=$gr; lastStartIndex=$lastStartIndex; text=$text; link=$link; title=$title; element: $element")
                ...
                10 -> {
                    ...
                    lastStartIndex = endIndex
                    //logd("when group=$gr; lastStartIndex=$lastStartIndex; text=$text; subs=$subs; order=$order; element: $element")
                ...
                11 -> {
                    ...
                    //logd("when group=$gr; lastStartIndex=$lastStartIndex; text=$text;  element: ${parents.last()}")
        ...
        if (lastStartIndex < string.length) {
            ...
            //logd("4 lastStartIndex < string.length; lastStartIndex=$lastStartIndex; text=$text; element: ${parents.last()}")
        }
        //logd("5 return parents: $parents")
        return parents
    }
}
